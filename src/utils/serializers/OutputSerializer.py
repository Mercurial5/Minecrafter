...
class OutputSerializer:
    ...
    def __init__(self, validated_data: Document):
        self.fields = {field[0]: field[1] for field in getmembers(self) if isinstance(field[1], BaseField)}
        self.validated_data = validated_data

    def prepare_output(self) -> dict | list[dict]:
        if isinstance(self.validated_data, Document):
    ...
    def _prepare_item_output(self, validated_data: Document) -> dict:
        output_data = dict()
